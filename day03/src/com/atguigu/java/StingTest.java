 ...
 *
 * @author kasio
 * @create 2020-05-2020/05/01 15:03
 */
public class StingTest {
    ...
    public void Test3(){
        ...
        //无论s5是在堆空间还是常量池中的对象，intern()方法的返回值强制要求区常量池中找
    ...
    面试题：String s3 = new String("abc");方式创建对象，在内存中创建了几个对象？
           答；两个，一个是在堆空间中new结构，另一个是char[]型数组对应的常量池中的数据："abc"
     */
    ...
    4.String：代表不可变的字符序列。简称：不可变性。
        体现：1.当对字符串重新赋值是，需要重写指定内存区域赋值，不能使用原有的value进行赋值。
             2.当对现有的进行拼接操作时，也需要重新指定内存区域赋值，不能使用原有的value进行赋值。
             3.当调用String的replace()方法修改指定字符或字符串时，也需要重新指定内存区域

    5.通过字面量的方式(区别于new)给一个字符串赋值，此时的字符串值得声明在字符串常量池中。
    6，字符串常量池中是不不会存储相同内容得字符串得。
