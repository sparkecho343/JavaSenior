 ...
 *   重写两个方法的小技巧：对象中用作 equals() 方法比较的 Field，都应该用来计算 hashCode 值。(两个方法内参与计算的属性一致)
 *
 * @author kasio
 * @create 2021-01-23 17:35
 */
public class SetTest {
    ...
    二，添加元素的过程：以HashSet为例：
        我们向HashSet中添加元素a，首先调用元素a所在类的hashCode()方法，计算元素a的哈希值(根据元素对象里的属性计算出哈希值)，
        此哈希值接着通过某种算法计算出在HashSet底层数组中的存放位置(即为：索引位置)，
        判断此位置上是否已经有元素；
            如果此位置上没有其他元素，则元素a添加成功。  --->情况1
            如果此位置上有其他元素b(或以链表形式存在的多个元素)，则比较元素a与元素b的hash值：
                如果哈希值不相同，则元素a添加成功。  --->情况2
                如果哈希值相同，进而需要调用元素a所在类的equlas()方法：
                    equals()返回true，元素a添加失败，
                    equals()返回false，则元素a添加成功。  --->情况3
        对于添加成功的情况2和情况3而言：元素a与已经存在指定索引上的数据以链表的方式存储。
        jdk7：元素a放到数组中，指向原来的元素。
        jdk8：原来的元素在数组中，指向元素a。
        总结：七上八下(放在旧数据的上方还是下方，箭头都是↓)
